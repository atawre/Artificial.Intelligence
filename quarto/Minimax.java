	...
	public static void generateStates(Node node, boolean pieceSelection, int depth) {
		...
		if(pieceSelection){
			...
			for(QuartoPiece p : b.pieces) {
				...
				pieceSelection ^= true;  //this is move selection for opponent. used XOR.
			...
			for(int i = 0; i < Node.NUMBER_OF_ROWS; i++){
				for(int j = 0; j < Node.NUMBER_OF_COLUMNS; j++) {
					...
					if(depth==DEPTH-1){
						...
						int h = t.heuristic();
						t.setValue(h);
						t.getBoard().insertPieceOnBoard(i, j, node.getPieceId());
						t.row = i; t.column = j;
						node.addChild(t);
						continue;
					}
					...
					pieceSelection ^= true; //this is piece selection for opponent. used XOR.
					c.setAction(pieceSelection);
					c.row = i;
					c.column = j;
					node.addChild(c);
